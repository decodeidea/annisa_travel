...
class Setting_system extends DC_controller {
	...
	function menu(){
		...
		$data['function']='menu';
		$list=select_where($this->tbl_menu,'lang',$this->lang->lang())->result();
			foreach ($list as $key) {
				if($key->sub_menu!=0){
	...
	function menu_form($id=null){
        ...
        $data['submenu']=select_where_array($this->tbl_menu, $arrayName = array('sub_menu' => 0,'lang'=>$this->lang->lang()))->result();
